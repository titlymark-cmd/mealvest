...
import { Search, Mail, Wallet, GraduationCap } from "lucide-react";
...
import { badgeColorFor, formatKsh } from "../../components/shared/format";
import { fetchHotelStudents, HotelStudent } from "../../services/hotelStaffApi";

const DISPLAY_LIMIT = 30;

export default function HotelStudentsScreen() {
  const { authFetch } = useAuth();
  const { width } = useWindowSize();
  const isMobile = width < HOTEL_MOBILE_BREAKPOINT;

  const [students, setStudents] = useState<HotelStudent[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const data = await fetchHotelStudents(authFetch);
      setStudents(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not load your students.");
    } finally {
      setLoading(false);
    }
  }, [authFetch]);
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.subtitle}>
        {students.length} student{students.length === 1 ? "" : "s"} contracted with this hotel.
      </p>
      ...
      {filtered.length === 0 ? (
        <Card style={styles.emptyCard}>
          ...
          <p style={styles.emptyText}>
            {students.length === 0 ? "No students have a plan with you yet." : `No students match "${query}".`}
          </p>
        </Card>
      ) : (
              ...
              <Card key={s.id} style={styles.studentCard}>
                ...
                {hasPlan ? (
                  ...
                  <div style={styles.noPlanBox}>
                    <span style={styles.noPlanText}>Contract not active</span>
                  </div>
                )}
              </Card>
            );
