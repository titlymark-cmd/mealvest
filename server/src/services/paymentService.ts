...
export async function activatePaymentIfNeeded(
  ...
  try {
    ...
    // Budget creation happens OUTSIDE the transactions-table
    // transaction on purpose — createBudget has its own internal
    // "one active budget per user" check and its own atomic insert;
    // nesting it inside this one risks a harder-to-reason-about
    // combined transaction for no real benefit, since the payment
    // being marked 'success' is already durably committed by now.
    await createBudget({
      userId: tx.user_id,
